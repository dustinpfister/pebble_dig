...
getRandomUserLand = function (username,done, fail) {
	...
	console.log('yes we are getting it: ' + username);
        ...
        if (err) {
            ...
            if (lands) {
                ...
                if (options.length > 0) {
                    ...
                    done(lands[Math.floor(Math.random() * len)]);
...
exports.newGame = function (type, scope, username, done, fail) {
	...
	console.log('the value of username in land.newgame is: ' + username);
	
    if (type = 'pvp') {

        getRandomUserLand(username,function (land) {
